#!/bin/bash
# Usage: run from terraform/ root or terraform/scripts/.
# The inventory filename is derived from the cluster_name Terraform output.
set -euo pipefail
...

{
  ...
  echo "ansible_ssh_private_key_file=~/.ssh/ntegra-k8s-key"
